...
public class SmsService{
   ...
   public void makeMassage() throws Exception{
      ...
      JSONObject toJson = new JSONObject();
       ...
       toJson.put("subject","[forREST]");                     // Optional, messages.subject   개별 메시지 제목, LMS, MMS에서만 사용 가능
       toJson.put("to","01033294534");                  // Mandatory(필수), messages.to   수신번호, -를 제외한 숫자만 입력 가능
       toArr.put(toJson);
       
       bodyJson.put("type","SMS");                     // Madantory, 메시지 Type (SMS | LMS | MMS), (소문자 가능)
       bodyJson.put("from","01033294534");               // Mandatory, 발신번호, 사전 등록된 발신번호만 사용 가능      
       bodyJson.put("content","인증번호 : ");   // Mandatory(필수), 기본 메시지 내용, SMS: 최대 80byte, LMS, MMS: 최대 2000byte
